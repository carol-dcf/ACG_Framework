...
void PhongMaterial::setUniforms(Camera* camera, Matrix44 model)
{
	...
	shader->setUniform("u_id", Application::instance->light_list[0]->difuse);
	shader->setUniform("u_is", Application::instance->light_list[0]->specular);
	shader->setUniform("u_light_pos", Application::instance->light_list[0]->position);

	if (texture)
		shader->setUniform("u_texture", texture);

	//shader->setUniform("u_normal_texture", nullptr);
	if (normal_texture)
